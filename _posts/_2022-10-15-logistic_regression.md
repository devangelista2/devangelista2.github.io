...
categories: [Lab4, teaching]
tags: [gradient descent, optimization, minimization, machine learning, stochastic gradient descent, logistic regression]
math: true
---
## Probabilistic Approach to (Binary) Classification
In the post dedicated to <a href="{{ site.baseurl }}{% link _posts/2022-09-28-classification_with_clustering.md %}">classification with clustering</a>, we saw that most of the classification algorithms reduce to defining *decision boundaries* and classifying based on that. Another possiblity is the **probabilistic approach**.

To simplify the explanation, assume we are working in a **binary** classification setup, i.e. where the number of classes $K = 2$. In this case, we can always represent the two classes as $y=0$ and $y=1$.
...
![](/assets/images/logistic_regression/sigmoid.png)

Since the output of $f_w(x)$ is continuous, it is natural to select, as loss function, the Mean Squared Error, as defined in the <a href="{{ site.baseurl }}{% link _posts/2022-10-14-stochastic_gradient_descent.md %}">previous post</a>. Indeed, the loss function becomes
