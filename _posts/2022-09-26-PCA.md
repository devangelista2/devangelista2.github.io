...
tags: [PCA, clustering, dimensionality reduction, SVD, unsupervised learning]
math: true
---
...
The first step is to load the dataset into memory, this can be done with Pandas as we saw into the <a href="{{ site.baseurl }}{% link _posts/2022-09-16-introduction_to_python_numpy.md %}">introductory post</a>.
...
Before implementing the algorithm performing PCA, you are required to split the dataset into a training set and a test set, following the <a href="{{ site.baseurl }}{% link _posts/2022-09-25-short_introduction_to_ML.md %}">introductory post on Machine Learning</a>. Remember that to correctly split $X$, it has to be random. 
